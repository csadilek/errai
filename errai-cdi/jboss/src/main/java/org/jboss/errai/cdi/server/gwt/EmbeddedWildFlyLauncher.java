...
public class EmbeddedWildFlyLauncher extends ServletContainerLauncher {
  ...
  public ServletContainer start(final TreeLogger treeLogger, final int port, final File appRootDir) throws BindException, Exception {
    ...
    try {
      ...
      File deploymentDir = prepareDeployment(appRootDir);
      JBossServletContainerAdaptor controller = new JBossServletContainerAdaptor(port, deploymentDir, 
  ...
  private File prepareDeployment(File appRootDir) throws IOException {
    ...
    for (File clientLocalClass : FileUtils.listFiles(appRootDir, new ClientLocalFileFilter(), TrueFileFilter.INSTANCE)) {
      String className = clientLocalClass.getAbsolutePath().replace(classesDir.getAbsolutePath(), "")
              .replace(".class", "").replace(File.separator, ".").substring(1);
      exclusions.append(DEVMODE_BEANS_XML_EXCLUSION_TEMPLATE.replace("$CLASSNAME", className));
    }
    String beansXml = DEVMODE_BEANS_XML_TEMPLATE.replace("$EXCLUSIONS", exclusions.toString());
    FileUtils.write(new File(webInfDir, "beans.xml"), beansXml);
    return appRootDir;
  }
